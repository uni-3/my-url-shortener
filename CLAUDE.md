...
**直接importするパッケージは必ず package.json に宣言すること**（phantom dependency禁止）。
推移的依存（例: `@opentelemetry/otlp-exporter-base`, `@opentelemetry/otlp-transformer`）を未宣言のままimportすると、ローカルでは node_modules のhoist残骸で動いてしまうが、CIのクリーンインストールでは別インスタンス解決になり型エラーでビルドが落ちる（PR #53で約3週間デプロイが止まっていた原因）。ローカル検証はリポジトリ外への隔離コピー + `pnpm install --frozen-lockfile` でないとCIを再現できない点にも注意。
